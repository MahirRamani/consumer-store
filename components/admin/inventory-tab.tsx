...
import { ApiResponse, InventoryLogWithDetails, LowStockSubProduct, QueryError, SubProduct } from "@/lib/types" // Updated import
...
export default function InventoryTab() {
  ...
  const { data: response, isLoading } = useQuery<ApiResponse<InventoryLogWithDetails[]>>({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {
        const error: QueryError = { message: "Failed to fetch inventory logs", status: response.status }
        throw error
      }
  ...
  // Updated to fetch SubProducts instead of Products
  const { data: lowStockProducts, isLoading: lowStockLoading } = useQuery<LowStockSubProduct[]>({
    queryKey: ["low-stock-products"],
    queryFn: async () => {
      const response = await fetch("/api/products/low-stock")
      // if (!response.ok) throw new Error("Failed to fetch low stock products")
      if (!response.ok) {
        const error: QueryError = { message: "Failed to fetch low stock products", status: response.status }
        throw error
      }
      return response.json()
    },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Low Stock Alert - Updated to show SubProduct details */}
        ...
        <Card className="border-red-200 bg-red-50">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {lowStockProducts.slice(0, 6).map((subProduct: LowStockSubProduct) => (
                <div key={subProduct.id} className="bg-white p-3 rounded-lg border border-red-200">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-medium text-gray-900">{subProduct.name}</p>
                      <p className="text-sm text-gray-600">Size: {subProduct.size}</p>
                      <p className="text-xs text-gray-500 capitalize">{subProduct.parentProduct?.category?.name?.replace("-", " ")}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-red-600">{subProduct.stock} left</p>
                      <p className="text-xs text-gray-500">Min: {subProduct.lowStockThreshold}</p>
                      <p className="text-xs text-gray-600">₹{subProduct.price}</p>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {/* Inventory Logs - Updated to show SubProduct details */}
      <Card>
        <CardHeader>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {inventoryLogs?.map((log: InventoryLogWithDetails) => (
                  <tr key={log.id}>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {log.product?.size && (
                              <>
                                ...
                                <p className="text-sm text-gray-500">Size: {log.product.size}</p>
                              </>
                            )}
                            {log.product?.price && (
                              <>
                                <span className="text-gray-300">•</span>
                                <p className="text-sm text-gray-500">₹{log.product.price}</p>
                              </>
                            )}
                          </div>
                        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div>
                        <p>{new Date(log.createdAt).toLocaleDateString('en-GB', {
                          day: '2-digit',
                          month: '2-digit',
                          year: 'numeric'
                        }) || "🤔?"}</p>
